...
typedef struct _meshtastic_HamParameters {
  ...
  char call_sign[8];
  /* Transmit power in dBm at the LoRA transceiver, not including any
   * amplification */
  int32_t tx_power;
  /* The selected frequency of LoRA operation
Please respect your local laws, regulations, and band plans.
Ensure your radio is capable of operating of the selected frequency before
setting this. */
  float frequency;
...
/* This message is handled by the Admin module and is responsible for all
 ...
 operations to both remote AND local nodes. (Prior to 1.2 these operations were
 done via special ToRadio operations) */
typedef struct _meshtastic_AdminMessage {
  pb_size_t which_payload_variant;
  union {
    /* Send the specified channel in the response to this message
 NOTE: This field is sent with the channel index + 1 (to ensure we never try to
 send 'zero' - which protobufs treats as not present) */
 ...
 If the client sets a particular channel to be primary, the previous channel
 will be set to SECONDARY automatically. */
    ...
    /* Begins an edit transaction for config, module config, owner, and channel
 settings changes This will delay the standard *implicit* save to the file
 system and subsequent reboot behavior until committed (commit_edit_settings) */
 ...
 to cancel reboot) Only Implemented for ESP32 Devices. This needs to be issued
 to send a new main firmware via bluetooth. */
 ...
 If received the simulator will exit successfully. */
    ...
    /* Tell the node to factory reset, all device settings will be returned to
     * factory defaults. */
...
#define _meshtastic_AdminMessage_ConfigType_MIN                                \
  meshtastic_AdminMessage_ConfigType_DEVICE_CONFIG
#define _meshtastic_AdminMessage_ConfigType_MAX                                \
...
#define _meshtastic_AdminMessage_ModuleConfigType_MIN                          \
  meshtastic_AdminMessage_ModuleConfigType_MQTT_CONFIG
#define _meshtastic_AdminMessage_ModuleConfigType_MAX                          \
  meshtastic_AdminMessage_ModuleConfigType_REMOTEHARDWARE_CONFIG
#define _meshtastic_AdminMessage_ModuleConfigType_ARRAYSIZE                                                    \
  ((meshtastic_AdminMessage_ModuleConfigType)(meshtastic_AdminMessage_ModuleConfigType_REMOTEHARDWARE_CONFIG + \
                                              1))

#define meshtastic_AdminMessage_payload_variant_get_config_request_ENUMTYPE    \
  meshtastic_AdminMessage_ConfigType
#define meshtastic_AdminMessage_payload_variant_get_module_config_request_ENUMTYPE \
  meshtastic_AdminMessage_ModuleConfigType
...
#define meshtastic_AdminMessage_get_canned_message_module_messages_request_tag \
  10
#define meshtastic_AdminMessage_get_canned_message_module_messages_response_tag \
  11
...
#define meshtastic_AdminMessage_FIELDLIST(X, a)                                \
  X(a, STATIC, ONEOF, UINT32,                                                  \
    (payload_variant, get_channel_request, get_channel_request), 1)            \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, get_channel_response, get_channel_response), 2)          \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, get_owner_request, get_owner_request), 3)                \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, get_owner_response, get_owner_response), 4)              \
  X(a, STATIC, ONEOF, UENUM,                                                   \
    (payload_variant, get_config_request, get_config_request), 5)              \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, get_config_response, get_config_response), 6)            \
  X(a, STATIC, ONEOF, UENUM,                                                   \
    (payload_variant, get_module_config_request, get_module_config_request),   \
    7)                                                                         \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, get_module_config_response, get_module_config_response), \
    8)                                                                         \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, get_canned_message_module_messages_request,              \
     get_canned_message_module_messages_request),                              \
    10)                                                                        \
  X(a, STATIC, ONEOF, STRING,                                                  \
    (payload_variant, get_canned_message_module_messages_response,             \
     get_canned_message_module_messages_response),                             \
    11)                                                                        \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, get_device_metadata_request,                             \
     get_device_metadata_request),                                             \
    12)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, get_device_metadata_response,                            \
     get_device_metadata_response),                                            \
    13)                                                                        \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, get_ringtone_request, get_ringtone_request), 14)         \
  X(a, STATIC, ONEOF, STRING,                                                  \
    (payload_variant, get_ringtone_response, get_ringtone_response), 15)       \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, get_device_connection_status_request,                    \
     get_device_connection_status_request),                                    \
    16)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, get_device_connection_status_response,                   \
     get_device_connection_status_response),                                   \
    17)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE, (payload_variant, set_ham_mode, set_ham_mode),  \
    18)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE, (payload_variant, set_owner, set_owner), 32)    \
  X(a, STATIC, ONEOF, MESSAGE, (payload_variant, set_channel, set_channel),    \
    33)                                                                        \
  X(a, STATIC, ONEOF, MESSAGE, (payload_variant, set_config, set_config), 34)  \
  X(a, STATIC, ONEOF, MESSAGE,                                                 \
    (payload_variant, set_module_config, set_module_config), 35)               \
  X(a, STATIC, ONEOF, STRING,                                                  \
    (payload_variant, set_canned_message_module_messages,                      \
     set_canned_message_module_messages),                                      \
    36)                                                                        \
  X(a, STATIC, ONEOF, STRING,                                                  \
    (payload_variant, set_ringtone_message, set_ringtone_message), 37)         \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, begin_edit_settings, begin_edit_settings), 64)           \
  X(a, STATIC, ONEOF, BOOL,                                                    \
    (payload_variant, commit_edit_settings, commit_edit_settings), 65)         \
  X(a, STATIC, ONEOF, INT32,                                                   \
    (payload_variant, reboot_ota_seconds, reboot_ota_seconds), 95)             \
  X(a, STATIC, ONEOF, BOOL, (payload_variant, exit_simulator, exit_simulator), \
    96)                                                                        \
  X(a, STATIC, ONEOF, INT32,                                                   \
    (payload_variant, reboot_seconds, reboot_seconds), 97)                     \
  X(a, STATIC, ONEOF, INT32,                                                   \
    (payload_variant, shutdown_seconds, shutdown_seconds), 98)                 \
  X(a, STATIC, ONEOF, INT32, (payload_variant, factory_reset, factory_reset),  \
    99)                                                                        \
  X(a, STATIC, ONEOF, INT32, (payload_variant, nodedb_reset, nodedb_reset), 100)
#define meshtastic_AdminMessage_CALLBACK NULL
#define meshtastic_AdminMessage_DEFAULT NULL
#define meshtastic_AdminMessage_payload_variant_get_channel_response_MSGTYPE   \
  meshtastic_Channel
#define meshtastic_AdminMessage_payload_variant_get_owner_response_MSGTYPE     \
  meshtastic_User
#define meshtastic_AdminMessage_payload_variant_get_config_response_MSGTYPE    \
  meshtastic_Config
#define meshtastic_AdminMessage_payload_variant_get_module_config_response_MSGTYPE \
  meshtastic_ModuleConfig
#define meshtastic_AdminMessage_payload_variant_get_device_metadata_response_MSGTYPE \
  meshtastic_DeviceMetadata
#define meshtastic_AdminMessage_payload_variant_get_device_connection_status_response_MSGTYPE \
  meshtastic_DeviceConnectionStatus
#define meshtastic_AdminMessage_payload_variant_set_ham_mode_MSGTYPE           \
  meshtastic_HamParameters
#define meshtastic_AdminMessage_payload_variant_set_owner_MSGTYPE              \
  meshtastic_User
#define meshtastic_AdminMessage_payload_variant_set_channel_MSGTYPE            \
  meshtastic_Channel
#define meshtastic_AdminMessage_payload_variant_set_config_MSGTYPE             \
  meshtastic_Config
#define meshtastic_AdminMessage_payload_variant_set_module_config_MSGTYPE      \
  meshtastic_ModuleConfig
